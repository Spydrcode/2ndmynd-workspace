...
function makeGroundingRoot(snapshot: SnapshotV2) {
  return {
    signals: {
      activity_signals: snapshot.activity_signals,
      quotes: snapshot.activity_signals.quotes,
      invoices: snapshot.activity_signals.invoices,
      volatility_band: snapshot.volatility_band,
      season: snapshot.season,
      input_costs: snapshot.input_costs,
    },
  };
}

export type EvidenceLeafCandidate = { path: string; value: string };
...
function collectLeafCandidates(node: unknown, pathBase: string, out: EvidenceLeafCandidate[]) {
  if (isLiteralValue(node)) {
    if (pathBase) {
      out.push({ path: pathBase, value: literalToString(node) });
    }
    return;
  }
  if (Array.isArray(node)) {
    for (let i = 0; i < node.length; i += 1) {
      collectLeafCandidates(node[i], `${pathBase}.${i}`, out);
    }
    return;
  }
  if (node && typeof node === "object") {
    ...
    for (const [key, value] of entries) {
      const nextPath = pathBase ? `${pathBase}.${key}` : key;
      collectLeafCandidates(value, nextPath, out);
    }
  }
}

export function listEvidenceLeafCandidates(snapshot: SnapshotV2): EvidenceLeafCandidate[] {
  const root = makeGroundingRoot(snapshot);
  const candidates: EvidenceLeafCandidate[] = [];
  collectLeafCandidates(root, "", candidates);
  return candidates.filter((candidate) => candidate.path.startsWith("signals."));
}
...
export function validateEvidenceSignalsAgainstSnapshotV2(
  ...
  for (let i = 0; i < evidenceSignals.length; i += 1) {
    ...
    const actual = getByPath(root, rawPath);
    if (actual === undefined) {
